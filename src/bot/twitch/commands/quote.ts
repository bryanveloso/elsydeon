...
export const quote = createBotCommand('quote', async (params, { say, msg: { userInfo } }) => {
  try {
    ...
    if (params.length === 0) {
      ...
      if (regex.test(quoteText)) {
        ...
        say(`I has OCD and can't accept that quote. Please format it like so: "<quote>" ~ @username`)
        return
      }
      ...
      const isSelf =
        username.toLowerCase() === userInfo.displayName.toLowerCase() ||
